...
import { CheckCircle2, Mail, Lock, User, ArrowRight, Search } from 'lucide-react';
...
const Login = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      setLoading(true);
      try {
        const res = await api.post('/auth/login', { ...values, role: userType });
          ...
          <div className="w-full max-w-md">
            ...
            <form className="space-y-6" onSubmit={formik.handleSubmit}>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder="••••••••" 
                    className={`w-full border rounded-lg p-3 pl-10 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${formik.touched.password && formik.errors.password ? 'border-red-500' : 'border-gray-200'}`}
                  />
                </div>
              ...
              <button 
                type="submit"
                disabled={loading}
                className="w-full bg-[#0F172A] text-white py-4 rounded-lg font-bold hover:bg-[#1E293B] transition-all transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed shadow-xl"
              >
